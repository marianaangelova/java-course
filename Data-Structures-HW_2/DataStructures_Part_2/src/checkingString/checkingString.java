...
public class checkingString {
	...
	public static void main(String[] args) {
		...
		if (isTrue) {
			for (int i = 0; i < expression.length(); i++) {
				if (i % 2 == 0) {
					queue.offer(Character.toString(expression.charAt(i)));
				}
				if(i % 2 != 0){
					myStack.push(Character.toString(expression.charAt(i)));
				}
			}
		}
		//unfinished...
		...
		String newString = queue.peek()+myStack.elementAt(0)+queue.peek();
